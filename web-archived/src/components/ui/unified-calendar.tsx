...
function UnifiedCalendar({
  ...
  // React 19対応：localeとしてオブジェクトをラップして使用
  const getSafeLocale = () => {
    return isJapanese ? ja : enUS;
  };
  ...
  const renderCalendar = () => {
    ...
    if (calendarMode === "single") {
      // singleモード用のハンドラ
      const handleSingleSelect: DayPickerSingleProps["onSelect"] = (
        newDate,
      ) => {
        if (onDateChange) {
      ...
      // multipleモード用のハンドラ
      const handleMultipleSelect: DayPickerMultipleProps["onSelect"] = (
        dates,
      ) => {
    ...
    <Popover open={isOpen} onOpenChange={setIsOpen}>
      <PopoverTrigger asChild>
        ...
        >
          ...
          {date ? (
            format(date, displayFormat, { locale: getSafeLocale() })
          ) : (
