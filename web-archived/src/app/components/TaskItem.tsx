...
const TaskItem = ({ task, isDraggable = false }: TaskItemProps) => {
  ...
  // トースト表示
  const showSuccessToast = (message: string = t("tasks.taskUpdated")) => {
  ...
  const showErrorToast = (message: string = t("tasks.taskUpdateFailed")) => {
    setToastType("error");
    setToastMessage(message);
    setShowToast(true);
    setTimeout(() => setShowToast(false), 3000);
  ...
  const handleCheck = async () => {
    try {
      ...
      if (newStatus === "done") {
        showSuccessToast(t("tasks.taskUpdated"));
      }
    } catch (err) {
    ...
    try {
      ...
      if (
        ...
        (!task.repeatSettings?.enabled ||
          JSON.stringify(task.repeatSettings) !==
            JSON.stringify(repeatSettings))
      ) {
        // 現在日から2週間分のタスクを生成
  ...
  const handleDelete = async () => {
    if (window.confirm(t("tasks.deleteConfirmation"))) {
      try {
        ...
        showSuccessToast(t("tasks.taskDeleted"));
      } catch (error) {
        console.error("Error deleting task:", error);
        showErrorToast(t("tasks.taskDeleteFailed"));
        setIsDeleting(false);
      }
    }
  };
  ...
  const getDayName = (day: number): string => {
    ...
    return locale === "en" ? daysEN[day] : daysJA[day];
  };
  ...
  const formatDisplayDate = (dateStr: string): string => {
    if (!dateStr) return t("tasks.unassigned");
    ...
    try {
      ...
      if (enableRepeat) {
        const repeatSettings: RepeatSettings = {
          ...
          endType: repeatEndType,
          occurrences:
            repeatEndType === "after" ? repeatOccurrences : undefined,
        ...
        <div className="flex items-center flex-1 min-w-0">
          <div className="relative flex items-start w-full">
            ...
            >
              ...
              {task.description && (
                <p
                  className={`text-xs truncate max-w-full sm:max-w-xs mt-0.5 ${checked ? "completed-task" : "text-gray-500"}`}
                >
                  ...
                  (task.reminderSettings.enableBlockStartReminder ||
                    ...
                    <div className="flex items-center">
                      ...
                      <span className="text-xs text-primary-500">
                        {t("tasks.reminderSettingsAvailable")}
                      </span>
                    </div>
                  )}
                {/* 繰り返し設定の表示 */}
                {task.repeatSettings?.enabled && (
                  <div className="flex items-center">
                    ...
                    <span className="text-xs text-green-600">
                      {t("tasks.repeatSettingsAvailable")}
                    </span>
                  </div>
                )}
              </div>
